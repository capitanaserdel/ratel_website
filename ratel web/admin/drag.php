...
$query = mysqli_query($config,"SELECT * FROM `opay_payment` WHERE reference in(SELECT reference FROM `opay_payment` GROUP BY reference HAVING COUNT(reference) > 1)");
$j=0;
while (($data = mysqli_fetch_assoc($query))){
  ...
  <tr>
      ...
      <td class="align-middle"><?php echo $data['ratelnumber']; ?></td>
     <td><?php echo $data['channels']; ?></td>
      <td><div class="badge badge-success badge-shadow"><?php echo $data['amount_r']; ?></div></td>
