...
if (!empty($pics1) && !empty($mobile)) {
    $query = mysqli_query($config, "INSERT INTO `registration`(mobile,calling_card,fname,sname,reference,gender,email,loc,nin,addr,pics1,pics2,pics3,amount,source,rc_number,date_of_inc,company_services,no_of_lline,staff,isPaid)VALUES('$mobile','$price','$fname','$sname','$reference','$gender','$email','$loc','$nin','$addr','$pics1','$pics2','$pics3','$amount','$source','$rc','$dateof','$compserv','$noL','$staff',1)")or die (mysqli_error($config));
       if ($query) {
        $last_id = mysqli_insert_id($config);
        echo "Success";
        echo    "<script>setTimeout(function(){

        $.redirect('subs/pay_sub.php', {formData:1,pics1:'$pics1',pics2:'$pics2',mobile:'$mobile',price:'$price',fname:'$fname',sname:'$sname',reference:$reference,gender:'$gender',email:'$email',loc:'$loc',nin:'$nin',addr:'$addr',amount:'$amount',lastID:'$last_id',source:'$source',staff:'$staff'});
        },3000);
        </script>";
}
}
